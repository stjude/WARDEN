...
FASTQSAMPLELIST = open("fastq_file_list.txt", 'w')
...
for line in SAMPLELIST:
    ...
    if len(read_file2) <= 1:
        ...
        if (read_file2 not in fastq_objects):
            PROCESSERRORS.write(read_file2 + " does not exist but is listed in the sample list file\n")
            PROCESSERRORS.close()
            sys.exit()
        if read_file2 in fastqs:
            PROCESSERRORS.write(read_file2 + " listed twice\n")
            PROCESSERRORS.close()
            sys.exit()
        fastqs.append(read_file2)
        object2 = fastq_objects[read_file2]

    object1 = fastq_objects[read_file1]
    FINALLIST.write("\t".join([sample_name, phenotype, read_file1, read_file2]) + "\n")
    FASTQSAMPLELIST.write(sample_name + "\t" + object1 + "\t" + object2 + "\n")
...
for filename in fastq_objects.keys():
    if filename not in fastqs:
        PROCESSERRORS.write("FASTQ file not found in sample file:" + filename + "\n")
        PROCESSERRORS.close()
        sys.exit()
