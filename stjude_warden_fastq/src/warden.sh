...
main() {
    ...
    printf '%s\n' "${FASTQ_FILES_path[@]}" > fastq_list.txt
    for fq in "${FASTQ_FILES[@]}"; do
        dx describe "$fq" --delim ',' | grep ID | cut -f 2 -d ',' >> fastq_link_list.txt
    done

    echo "###PROCESSING FILES###"
    process_files.py "true" sample_list.txt fastq_list.txt fastq_link_list.txt
    ...
    echo "fastq_file_list.txt:"
    cat fastq_file_list.txt
    ...
    ###############
    {
        ...
        echo -e "mark_duplicates\t$mark_duplicates"
        echo -e "generate_transcriptome_BAM\t$generate_transcriptome_BAM"
        echo -e "generate_name_sorted_BAM\t$generate_name_sorted_BAM"
        echo -e "outSAMunmapped\t$outSAMunmapped"
        echo -e "outSAMattributes\t$outSAMattributes"
        echo -e "outFilterMultimapNmax\t$outFilterMultimapNmax"
        echo -e "outFilterMismatchNmax\t$outFilterMismatchNmax"
        echo -e "alignIntronMax\t$alignIntronMax"
        echo -e "outSAMstrandField\t$outSAMstrandField"
        echo -e "intronMotif\t$intronMotif"
        echo -e "chimSegmentMin\t$chimSegmentMin"
        echo -e "sjdbOverhang\t$sjdbOverhang"
        echo -e "chimJunctionOverhangMin\t$chimJunctionOverhangMin"
        echo -e "STAR_subsample_n_reads\t$STAR_subsample_n_reads"
        echo -e "star_instance\t$star_instance"
    ...
    wf_id=$(create_workflow.py workflow_parameter_file.txt fastq_file_list.txt "$final_sample_list_id" "$comparisons_limma_id" "$comparisons_all_id")
    ...
    {
        echo "$(./jq-1.6 'with_entries(if (.key | test("_forward_fastqc_html")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "forward_fastqc_htmls": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_forward_fastqc_zip")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "forward_fastqc_zips": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_reverse_fastqc_html")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "reverse_fastqc_htmls": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_reverse_fastqc_zip")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "reverse_fastqc_zips": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_star_bam")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "star_bams": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_star_transcriptome_bam")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "star_transcriptome_bams": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("^(?!.*combined_).*_flagstat"; "")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "flagstats": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_star_splice_junctions")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "star_splice_junctions": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_star_chimeric_bam")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "star_chimeric_bams": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_star_chimeric_junction")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "star_chimeric_junctions": . }' analysis.json | head -n -1 | tail -n +2)",
        echo "$(./jq-1.6 'with_entries(if (.key | test("_name_sorted_bam")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "name_sorted_bams": . }' analysis.json | head -n -1 | tail -n +2)",
        ...
        echo "$(./jq-1.6 'with_entries(if (.key | test("combined_flagstat")) then ( {key: .key, value: .value } ) else empty end) | flatten | { "combined_flagstat": .[0] }' analysis.json | head -n -1 | tail -n +2)",
