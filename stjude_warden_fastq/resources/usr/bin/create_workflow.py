...
def build_workflow():
    ...
    fastqc_applet = dxpy.search.find_one_data_object(
        classname="applet",
        name=app_names["fastqc"],
        state="closed",
        return_handler=True,
    )
    star_applet = dxpy.search.find_one_data_object(
        classname="applet", name=app_names["star"], state="closed", return_handler=True
    )
    ...
    htseq_applet = dxpy.search.find_one_data_object(
        classname="applet", name=app_names["htseq"], state="closed", return_handler=True
    )
    ...
    bigwig_applet = dxpy.search.find_one_data_object(
        classname="applet",
    ...
    if parameters["two_pass_alignment"] == "true":
        for sample_name in samples:
            ...
            align_input["star_index_archive"] = dxpy.dxlink(
                {"project": index_project, "id": index_id}
            )
            if (
                parameters["sjdbFileChrStartEnd"] != "null"
                and parameters["sjdbFileChrStartEnd"] != ""
            ):
                ...
                align_input["sjdbFileChrStartEnd"] = dxpy.dxlink(
                    {
                        "project": sjdbFileChrStartEnd_project,
                        "id": sjdbFileChrStartEnd_id,
                    }
                )
            if parameters["indexed_with_gtf"] != "true":
                align_input["transcriptome_gtf"] = dxpy.dxlink(
                    {"project": gtf_project, "id": gtf_id}
                )
            ...
            sj_out_files_arr.append(
                dxpy.dxlink(
                    {"stage": first_align_stage_id, "outputField": "sj_tab_out"}
                )
            )
        ...
        parameters["pass1_sj_out"] = dxpy.dxlink(
            {"stage": combine_sj_out_stage_pass1_id, "outputField": "combined_sj_out"}
        )
    ...
    for sample_name in samples:
        ...
        if "pass1_sj_out" in parameters:
            ...
            align_input["sjdbFileChrStartEnd"] = dxpy.dxlink(
                {"project": sjdbFileChrStartEnd_project, "id": sjdbFileChrStartEnd_id}
            )
        align_input["output_prefix"] = sample_name
        align_stage_id = ""
        if samples[sample_name][1] != "-":
        ...
        flagstat_files_arr.append(
            dxpy.dxlink({"stage": align_stage_id, "outputField": "flagstat_out"})
        )
        sj_out_files_arr.append(
            dxpy.dxlink({"stage": align_stage_id, "outputField": "sj_tab_out"})
        )
        wf_outputs += [
            {
        ...
        if parameters["generate_name_sorted_BAM"] == "true":
            sort_input = {
                "input_bam": dxpy.dxlink(
                    {"stage": align_stage_id, "outputField": "sorted_by_coord_bam"}
                )
            }
            sort_stage_id = wf.add_stage(
                sort_bam_applet,
            ...
            htseq_input = {
                "input_bam": dxpy.dxlink(
                    {"stage": sort_stage_id, "outputField": "output_bam"}
                )
            }
        ...
        htseq_input["gene_length_file"] = dxpy.dxlink(
            {"project": gene_length_project, "id": gene_length_id}
        )
        ...
        htseq_results.append(
            dxpy.dxlink({"stage": htseq_stage_id, "outputField": "htseq_counts"})
        )
        wf_outputs += [
            {
        ...
        if parameters["id_attribute"] == "gene_name":
            fpkm_results.append(
                (dxpy.dxlink({"stage": htseq_stage_id, "outputField": "fpkm"}))
            )
            fpkm_log2_results.append(
                (dxpy.dxlink({"stage": htseq_stage_id, "outputField": "fpkm_log2"}))
            )
            wf_outputs += [
                {
        ...
        if parameters["run_coverage"] == "true":
            ...
            bg2bw_all_input["bedgraph_file"] = dxpy.dxlink(
                {"stage": gcb_stage_id, "outputField": "all_coverage_file"}
            )
            ...
            bigwig_files.append(
                dxpy.dxlink({"stage": bg2bw_all_stage_id, "outputField": "bigwig"})
            )
            wf_outputs += [
                {
            ...
            if parameters["strandedness"] != "no":
                ...
                bg2bw_pos_input["bedgraph_file"] = dxpy.dxlink(
                    {"stage": gcb_stage_id, "outputField": "pos_coverage_file"}
                )
                ...
                bg2bw_neg_input["bedgraph_file"] = dxpy.dxlink(
                    {"stage": gcb_stage_id, "outputField": "neg_coverage_file"}
                )
                ...
                bigwig_files.append(
                    dxpy.dxlink({"stage": bg2bw_pos_stage_id, "outputField": "bigwig"})
                )
                bigwig_files.append(
                    dxpy.dxlink({"stage": bg2bw_neg_stage_id, "outputField": "bigwig"})
                )
        sample_num += 1
    ...
    if parameters["BW_VIEWER"] != "None" and parameters["run_coverage"] == "true":
        ...
        bw_viewer_stage_id = wf.add_stage(
            bw_viewer_applet,
    ...
    if parameters["run_simple_dif_ex"] == "true":
        simple_DE_input = {
            ...
            "difex_viewer": limma_viewer_link,
        }
        if parameters["limma_DE_viewer"] != "None":
