...
main() {
    ...
    samtools_command="samtools flagstat $input_bam_path > unsplit.flagstat.txt"
    ...
    genome_coverage_command="bedtools genomecov $output_options -scale $scale -split -ibam $input_bam_path  -g $genome_sizes_file_path > coverage_file.bed"
    ...
    if [ "$strandedness" != "no" ]; then
        samtools view -h -f 80 "$input_bam_path" > s_r1.bam
        samtools view -h -f 160 "$input_bam_path" > s_f2.bam
        samtools view -h -f 96 "$input_bam_path" > as_f1.bam
        samtools view -h -f 144 "$input_bam_path" > as_r2.bam
